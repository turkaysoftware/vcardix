        ...
        public void LoadVcf(string filePath){
            ...
            foreach (var block in vcardBlocks){
                ...
                for (int i = 0; i < block.Count; i++){
                    ...
                    if (line.StartsWith("N:", StringComparison.OrdinalIgnoreCase) || line.StartsWith("N;", StringComparison.OrdinalIgnoreCase)){
                        ...
                        if (DateTime.TryParse(line.Substring(5).Trim(), out DateTime dt)) current.Birthday = dt;
                        ...
                        current.PhoneMobile = line.Substring(line.IndexOf(':') + 1).Trim();
                        ...
                        current.PhoneHome = line.Substring(line.IndexOf(':') + 1).Trim();
                        ...
                        current.PhoneWork = line.Substring(line.IndexOf(':') + 1).Trim();
                ...
                switch (CurrentVersion){
                    case VCardVersion.V21:
                        ...
                        if (!string.IsNullOrEmpty(c.PhoneMobile))
                            sb.AppendLine($"TEL;CELL:{EscapeTextV21(c.PhoneMobile)}");
                        if (!string.IsNullOrEmpty(c.PhoneHome))
                            sb.AppendLine($"TEL;HOME:{EscapeTextV21(c.PhoneHome)}");
                        if (!string.IsNullOrEmpty(c.PhoneWork))
                            sb.AppendLine($"TEL;WORK:{EscapeTextV21(c.PhoneWork)}");
